package com.dat3m.dartagnan.asm.builtins;
...
public class AsmLibvsyncBuiltinsTest {
    ...
    public AsmLibvsyncBuiltinsTest (String file, int bound, Result expected) {
        this.programPath = getTestResourcePath("asm/builtins/" + file + ".ll");
        this.bound = bound;
    ...
    public void testAllSolvers() throws Exception {
        // change solver as needed, I am runnning one at a time
        long start = System.currentTimeMillis();
        try (SolverContext ctx = mkCtx(); ProverWithTracker prover = mkProver(ctx)) {
            assertEquals(expected, RefinementSolver.run(ctx, prover, mkTask()).getResult());
        }
        long end = System.currentTimeMillis();
        System.out.println("File " + programPath + " took " + (end - start));
    }
